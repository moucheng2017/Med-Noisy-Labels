...
images, labels_AR, labels_HS, labels_SG, imagename = validate_dataset[Image_index_to_demonstrate]
...
model = UNet_GlobalCMs(in_ch=input_dim, width=width, depth=depth, class_no=class_no, 
                       input_height= i_height, input_width=i_width, annotators=3)
...
for epoch in range(num_epochs):
    ...
    for j, (images, labels_AR, labels_HS, labels_SG, imagename) in enumerate(trainloader):
        ...
        train_iou = segmentation_scores(labels_AR.cpu().detach().numpy(), train_output.cpu().detach().numpy(), class_no)
        running_loss += loss
